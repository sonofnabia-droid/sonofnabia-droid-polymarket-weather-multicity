...
warsaw_download.py
==================
Descarrega histórico horário para Warsaw (EPWA) via Wunderground API
...
def fetch_day(station: str, tz: str, day: date) -> list[dict]:
    ...
    url = f"https://api.weather.com/v1/location/{station}:9:PL/observations/historical.json"
...
# ── Update mode ────────────────────────────────────────────────────────────────
def update_mode():
    csv_path = OUTPUT_DIR / "warsaw.csv"
...
def main_download(start_date: date, end_date: date):
    print(f"{Fore.CYAN}╔══════════════════════════════════════╗")
    print(f"{Fore.CYAN}║ Wunderground – EPWA Warsaw (Chopin) ║")
    print(f"{Fore.CYAN}╚══════════════════════════════════════╝\n")
    ...
    with Progress(TextColumn("[bold cyan]{task.description}"), BarColumn(), TextColumn("{task.percentage:>3.0f}%"), TimeElapsedColumn(), TimeRemainingColumn()) as progress:
        task = progress.add_task(f"[EPWA] Warsaw", total=total_days)
        download_station(STATION, CITY, TZ, start_date, end_date, progress, task)

# ── CLI ────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download histórico Wunderground Warsaw")
