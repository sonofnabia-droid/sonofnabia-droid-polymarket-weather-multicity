...
====================
Backtester unificado (consolidação dos dois anteriores).

Inclui:
  • PnL realista Polymarket: (1/ask - 1) por $, não 0.95 fixo       (Fix #7)
  • Outcome real: "correct" = bracket cobre peak_temp                (Fix #7)
  • 1 parcela por slot em phased (paridade com live_bot)             (Fix #6)
  • RESUMO TOTAL acumulado                                           (Fix #5)
  • Sharpe/Sortino reais a partir de retornos diários                (Fix #8)
  • Estatísticas sazonais (winter/spring/summer/autumn)              ← do backtester antigo
  • Lag por parcela em horas (diagnóstico de timing)                 ← do backtester antigo
  • prev_7d_avg_max real por dia (não 15.0 hardcoded)                ← do backtester antigo
  • SimulatedMarket com ruído de mercado (quebra circularidade)      ← NOVO
  • Save de resumo JSON em backtest_results/                         ← do backtester antigo

Uso:
    python munich_backtester.py --mode both --years 3 --ordertype percent --bet 2
    python munich_backtester.py --mode single --years 5 --ordertype fixed --bet 5
    python munich_backtester.py --mode both --years 3 --noise 0.0      # determinístico
    python munich_backtester.py --mode both --years 3 --noise 0.08     # ruído realista (default)
"""
...
from munich_config import DATA_CSV, BACKTEST_RESULTS_DIR, DAY_START, DAY_END, MIN_HOUR, SEASONS
from munich_model import load_models, predict_ensemble
...
class SimulatedMarket:
    ...
    def __init__(self, temp_range=range(-5, 45), noise_std: float = 0.05, seed: int = 42):
...
def load_data(csv_path: Path = DATA_CSV) -> pd.DataFrame:
    """Carrega e normaliza o CSV histórico."""
    ...
    # Colunas meteorológicas (mapeamentos do CSV do utilizador)
    raw["humidity"] = pd.to_numeric(raw.get("humidity_pct"), errors="coerce").fillna(70.0)
    raw["cloud_cover"] = pd.to_numeric(raw.get("sky_cover"), errors="coerce").fillna(50.0)
    raw["dewpoint_c"] = pd.to_numeric(raw.get("dewpt_c"), errors="coerce").fillna(raw["temp_c"] - 10)
    raw["pressure_hpa"] = pd.to_numeric(raw.get("pressure_hpa"), errors="coerce").fillna(1013.0)
    raw["wind_dir_deg"] = pd.to_numeric(raw.get("wind_dir_deg"), errors="coerce").fillna(0.0)
    raw["wind_speed_kmh"] = pd.to_numeric(raw.get("wind_speed_kmh"), errors="coerce").fillna(5.0)
    raw["wind_gust_kmh"] = pd.to_numeric(raw.get("wind_gust_kmh"), errors="coerce").fillna(8.0)
    raw["uv_index"] = pd.to_numeric(raw.get("uv_index"), errors="coerce").fillna(3.0)
...
def _compute_prev7_map(df: pd.DataFrame) -> dict:
    """{date: prev_7d_avg_max} — usa mesmo fallback que compute_prev7."""
    daily_max = df.groupby("date")["temp_c"].max().sort_index()
    dates_list = list(daily_max.index)
    prev7 = {}
    for i, d in enumerate(dates_list):
        if i == 0:
            # Mesmo fallback climatológico que compute_prev7
            prev7[d] = _CLIMATOLOGY_BY_MONTH.get(d.month, 15.0)
        else:
            window = daily_max[dates_list[max(0, i - 7):i]]
            prev7[d] = float(window.mean()) if len(window) else _CLIMATOLOGY_BY_MONTH.get(d.month, 15.0)
    return prev7
...
def run_backtest(
    ...
    mode: "single", "phased" ou "both" — determina qual PnL é somado ao
    capital simulado. Antes era sempre single+phased o que destruía o capital
    quando uma das estratégias perdia.
    """
    sim_mkt = SimulatedMarket(noise_std=noise_std)
        ...
        for d, day_df in df.groupby("date"):
            ...
            if year not in yearly:
                yearly[year] = {
                    ...
                    "phased": {"correct":0,"premature":0,"missed":0,"total":0,"trades":0,"invested":0.0,"pnl":0.0}
                }
            ...
            # SingleEntry usa defaults calibrados (thr=0.68, hour_min=15h)
            entry_single = SingleEntry(parcel_size=bet_size)
            entry_phased = PhasedEntry(parcel_size=bet_size)
            ...
            for _, row in day_df.iterrows():
                ...
                # PHASED — 1 parcela por slot
                for act in entry_phased.evaluate(p_ens, h, market_sim, running_max, fc_agreement):
                    if act.get("size_usdc", 0) > 0:
                        best = max(brackets, key=lambda b: b["ask"])
                        entry_phased.mark_bought(act["parcel_idx"], {
                            "hour": h, "slot30": s,
                            "ask": best["ask"],
                            "bracket_label": best["label"],
                            "temp_lo": best["temp_lo"],
            ...
            # PHASED
            phased_won_any, phased_pnl = False, 0.0
            parcel_lags_h = [None, None, None]
            phased_had_premature = False
            for pidx, rec in enumerate(entry_phased.parcel_records):
                if not rec:
                    continue
                won = _bracket_contains_peak(rec["temp_lo"], rec["temp_hi"], peak_temp)
                phased_won_any = phased_won_any or won
                phased_pnl += rec["size_usdc"] * _pnl_per_dollar(rec["ask"], won)
                lag_slots = peak_sidx - _slot_idx(rec["hour"], rec["slot30"])
                parcel_lags_h[pidx] = lag_slots * 0.5
                if rec["hour"] < peak_h and not won:
                    phased_had_premature = True
            correct_p = phased_won_any
            premature_p = phased_had_premature
            missed_p = not any(entry_phased.parcel_bought)

            # Acumular anual
            for mode, correct, premature, missed, invested, pnl in [
                ("single", correct_s, premature_s, missed_s, entry_single.total_invested, single_pnl),
                ("phased", correct_p, premature_p, missed_p, entry_phased.total_invested, phased_pnl),
            ]:
                yearly[year][mode]["correct"]   += int(correct)
                yearly[year][mode]["premature"] += int(premature)
                yearly[year][mode]["missed"]    += int(missed)
                yearly[year][mode]["total"]     += 1                # total de DIAS (todos)
                # Trades reais = dias com entrada efectiva (correct OR premature, NOT missed)
                yearly[year][mode]["trades"]    += int(not missed)
                yearly[year][mode]["invested"]  += invested
                yearly[year][mode]["pnl"]       += pnl
            ...
            day_records.append({
                ...
                "phased_correct": correct_p, "phased_premature": premature_p,
                "phased_missed": missed_p, "phased_pnl": phased_pnl,
                "parcel1_bought": entry_phased.parcel_bought[0],
                "parcel2_bought": entry_phased.parcel_bought[1],
                "parcel3_bought": entry_phased.parcel_bought[2],
                "parcel1_lag_h": parcel_lags_h[0],
                "parcel2_lag_h": parcel_lags_h[1],
                "parcel3_lag_h": parcel_lags_h[2],
                "phased_n_parcels": sum(entry_phased.parcel_bought),
                "phased_invested": entry_phased.total_invested,
            })

            if ordertype == "percent":
                # Só somar ao capital o PnL relevante para o modo escolhido.
                # Antes somava ambos, o que destruía o capital de SINGLE quando
                # PHASED estava a perder $ em paralelo.
                cap_before = capital
                if mode == "single":
                    capital += single_pnl
                elif mode == "phased":
...
def compute_stats(day_records: list, mode: str, capital_history: list) -> BacktestStats:
    ...
    if mode == "phased":
        stats.parcel1_pct = round(df["parcel1_bought"].mean() * 100, 1)
        stats.parcel2_pct = round(df["parcel2_bought"].mean() * 100, 1)
        stats.parcel3_pct = round(df["parcel3_bought"].mean() * 100, 1)
        stats.avg_n_parcels = round(df["phased_n_parcels"].mean(), 2)
        stats.avg_invested = round(df["phased_invested"].mean(), 2)
...
def print_dashboard(
    ...
    bet_value: float = 5.0,
    noise_std: float = 0.08,
    model_dir: Path = Path("munich_peak_model"),
) -> tuple:
    """Dashboard principal. Retorna (stats_single, stats_phased)."""
    ...
    _console.rule(f"[bold cyan]BACKTEST FINAL — SINGLE vs PHASED[/bold cyan]")
    ...
    for col, w, just in [("Ano",6,"left"),("Modo",8,"left"),
                          ("Trades",7,"right"),("Win%",7,"right"),("Wins",6,"right"),
    ...
    total_phased = {"correct":0, "premature":0, "missed":0, "total":0, "trades":0, "invested":0.0, "pnl":0.0}

    for year in sorted(yearly):
        for mode in ["single", "phased"]:
            s = yearly[year][mode]
            t_days = s["total"]
            t_trades = s["trades"]
            ...
            tgt = total_single if mode == "single" else total_phased
            for k in ("correct","premature","missed","total","trades","invested","pnl"):
                tgt[k] += s[k]
        table.add_row("─"*6,"─"*8,"─"*7,"─"*7,"─"*6,"─"*7,"─"*9,"─"*10,"─"*11,"─"*8)
    ...
    for c in ["Modo","Dias","Trades","Win%","Wins","Prem%","NoEntry%","Invested","PnL $","PnL%"]:
        total_table.add_column(c, justify="right")
    for mode_name, data in [("SINGLE", total_single), ("PHASED", total_phased)]:
        t_days = data["total"]
    ...
    stats_phased = compute_stats(day_records, "phased", capital_history)

    for mode_name, stats in [("SINGLE", stats_single), ("PHASED", stats_phased)]:
        _console.rule(f"[dim]{mode_name} — Lag & Sazonalidade[/dim]", style="dim")

        if stats.lag_mean_h is not None:
            _console.print(
                f"  Lag: médio +{stats.lag_mean_h}h, mediano +{stats.lag_median_h}h, "
                f"≤1h: {stats.lag_le1h_pct}%, ≤2h: {stats.lag_le2h_pct}%"
            )

        if mode_name == "PHASED":
            _console.print(
                f"  Parcelas: P1={stats.parcel1_pct}%  P2={stats.parcel2_pct}%  P3={stats.parcel3_pct}%   "
                f"média/dia={stats.avg_n_parcels}  invested/dia=${stats.avg_invested:.2f}"
            )

        if stats.seasonal_stats:
            ...
            for season in ["winter", "spring", "summer", "autumn"]:
                ss = stats.seasonal_stats.get(season)
                if not ss: continue
    ...
    return stats_single, stats_phased
...
# ══════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Munich Backtester unificado")
    parser.add_argument("--mode", choices=["single","phased","both"], default="both")
    ...
    parser.add_argument("--model", choices=["normal","optuna"], default="normal")
    ...
    parser.add_argument("--noise", type=float, default=0.05,
                        help="Ruído gaussiano no ask do mercado simulado (default 0.05 = 5¢). "
                             "0.0 = determinístico. Na v2 climatológica, ~5¢ reflete melhor "
                             "a variação de liquidez e sentiment.")
    args = parser.parse_args()

    _console.print("\n[bold cyan]Munich Backtester[/bold cyan]")
    _console.print("[1/4] Modelos...")
    model_dir = Path("munich_peak_model_optuna") if args.model == "optuna" else Path("munich_peak_model")
    models = load_models(model_dir)

    _console.print("\n[2/4] Dados...")
    df_all = load_data(DATA_CSV)
    ...
    stats_single, stats_phased = print_dashboard(
    ...
    out_json = BACKTEST_RESULTS_DIR / f"munich_backtest_{args.mode}_{start_date.isoformat()}_{end_date.isoformat()}.json"
    out_json.write_text(json.dumps({
        ...
        "phased": {
            "total_days": stats_phased.total_days,
            "correct_pct": stats_phased.correct_pct,
            "premature_pct": stats_phased.premature_pct,
            "missed_pct": stats_phased.missed_pct,
            "lag_mean_h": stats_phased.lag_mean_h,
            "parcel1_pct": stats_phased.parcel1_pct,
            "parcel2_pct": stats_phased.parcel2_pct,
            "parcel3_pct": stats_phased.parcel3_pct,
            "total_pnl": stats_phased.total_pnl,
            "sharpe": stats_phased.sharpe,
            "sortino": stats_phased.sortino,
            "seasonal_stats": stats_phased.seasonal_stats,
        },
    }, indent=2))
    ...
    if capital_flow_debug:
        debug_csv = BACKTEST_RESULTS_DIR / f"capital_flow_debug_{args.mode}.csv"
        ...
        sum_phased = df_debug["phased_pnl"].sum()
        ...
        _console.print(
            ...
            f"  Soma de phased_pnl: [bold]${sum_phased:+,.2f}[/bold]\n"
