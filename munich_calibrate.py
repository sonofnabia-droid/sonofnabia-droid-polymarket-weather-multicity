...
munich_calibrate.py
===================
Análise de calibração de thresholds para SingleEntry (foco 2026-04).

Corre o modelo treinado sobre os dados históricos e faz grid search
sobre (threshold, hour_min), avaliando:
  - Win rate (acertar o bracket do pico)
  - Trade frequency
  - Lag mediano (entrada vs pico real)
  - PnL total (payoff Polymarket realista)
  - Estabilidade (quão sensível é o ROI a pequenas mudanças nos params)

No fim apresenta 3 perfis de configuração prontos a colar no
munich_phased_entry.py (classe SingleEntry):
  🛡 CONSERVADOR  — máxima confiança, menos trades, win-rate alto
  ⚖ BALANCEADO   — melhor equilíbrio entre volume e precision
  🚀 AGRESSIVO    — mais trades, maior ROI absoluto, mais variância
...
Métricas (--metric):
  roi      Optimiza ROI $ via SimulatedMarket [DEFAULT, retro-compat]
           Depende de preços inventados pelo simulador climatológico.
           Útil para visualizar lucro estimado mas enviesado.

  outcome  Optimiza win-rate × log(volume) — métrica HONESTA.
           Não usa preços simulados. Foca-se na qualidade do sinal:
           win%, premature%, late%, lag. Recomendado para escolha
           de configs robustas, sem enviesar pelo SimulatedMarket.

Flags:
  --realistic      Usa SimulatedMarket com ruído (só relevante p/ --metric roi)
  --top N          Top-N resultados a mostrar (default 30)
  --years N        Quantos anos de histórico usar (default 3, recomendado 5)

Uso:
    # Modo financeiro (legacy, com preços simulados)
    python munich_calibrate.py --years 5 --mode full --realistic --metric roi

    # Modo honesto (recomendado para optimização)
    python munich_calibrate.py --years 5 --mode full --metric outcome
"""
import argparse
...
from munich_config import DATA_CSV, DAY_START, DAY_END, MIN_HOUR
from munich_model import load_models, predict_ensemble
from munich_backtester import (
    load_data, _compute_prev7_map, _bracket_contains_peak,
    _pnl_per_dollar, SimulatedMarket,
)
...
# ══════════════════════════════════════════════════════
def generate_daily_signals(df: pd.DataFrame, models: dict) -> pd.DataFrame:
        ...
        for d, day_df in df.groupby("date"):
            ...
            for _, row in day_df.iterrows():
                ...
                if h < MIN_HOUR or len(slots_so_far) < 4:
                    ...
                    current_extra = {**slot_entry, "prev_7d_avg_max": prev7}
...
# ══════════════════════════════════════════════════════
def simulate_strategy(signals_df: pd.DataFrame, threshold: float,
                      hour_min: int, hour_max: int = 20,
    ...
    realistic_market=True usa SimulatedMarket (com ruído), igual ao backtester.
    realistic_market=False usa ask ≈ p_ensemble (proxy conservador).
    """
    PARCEL = 5.0
    total_days = 0
    trades = 0
    ...
    for d, day_df in signals_df.groupby("date"):
        ...
        # (mesma lógica do munich_backtester — corresponde ao que o live bot faz
        # quando olha ao mercado real).
        # ANTIGO (bugado): bracket_lo = round(entry_rmax); fazia match pelo valor
        #                  arredondado, mas quando rmax<5°C ou >39°C saía fora do
        #                  temp_range do SimulatedMarket e caía num fallback com
        #                  ask artificialmente baixo, gerando ROIs absurdos em
        #                  thresholds baixos.
        if realistic_market and market_sim is not None:
            ...
            # No modo proxy, mantemos a lógica simples — só importa para debug
            bracket_lo = round(entry_rmax)
            bracket_hi = round(entry_rmax)
            ask = max(0.05, min(0.95, p_at_entry))

        won = _bracket_contains_peak(bracket_lo, bracket_hi, peak_temp)

        pnl = PARCEL * _pnl_per_dollar(ask, won)
    ...
    return {
        ...
        # ═══════════════════════════════════════════════════
        # MÉTRICA OUTCOME — independente de preços simulados
        # ═══════════════════════════════════════════════════
        # Score composto que recompensa win-rate alto MAS exige volume:
        #
        #   outcome_score = win_pct × log10(1 + trades_per_year)
        #
        # Exemplos:
        #   100 trades/ano @ 90% win  → 90 × log10(101)  = 180.4
        #    50 trades/ano @ 95% win  → 95 × log10(51)   = 162.3
        #    10 trades/ano @ 100% win → 100 × log10(11)  = 104.1
        #   200 trades/ano @ 85% win  → 85 × log10(201)  = 195.6
        #
        # log10 satura volume (não recompensa demasiado >300 trades/ano)
        # mas penaliza fortemente <30 trades/ano onde win% é estatisticamente
        # frágil. É honesto sobre a qualidade do sinal, sem inventar preços.
...
#  GRID SEARCH
# ══════════════════════════════════════════════════════
def run_grid_search(signals_df: pd.DataFrame, thresholds: list,
                    hour_mins: list, realistic_market: bool = False) -> list:
    market_sim = SimulatedMarket(noise_std=0.05, seed=42) if realistic_market else None
    results = []
    ...
    with Progress(
        ...
        for thr in thresholds:
            for hmin in hour_mins:
                r = simulate_strategy(signals_df, thr, hmin,
...
#  TABELAS E ANÁLISES
# ══════════════════════════════════════════════════════
def print_p_distribution(signals_df: pd.DataFrame) -> None:
...
def print_top_results(results: list, n: int = 30, sort_by: str = "roi_pct",
    ...
    if metric == "outcome":
        # Colunas focadas no outcome: sem $, sem AskWin/AskLoss
...
def print_heatmap(results: list, metric: str = "roi") -> None:
    ...
    if metric == "outcome":
        ...
        # Bandas para outcome_score (0-200 typically)
        def _colorize(v):
...
def print_stability_analysis(results: list, metric: str = "roi") -> None:
    ...
    # Selecionar coluna conforme métrica
    ...
    # Vizinhos dentro de +/- 0.05 no threshold e +/- 1h
    neighbors = []
    for r in viable:
    ...
    for _, _, r in sorted(neighbors, key=lambda x: (x[0], x[1])):
        ...
        # Threshold de "está mau" depende da escala da métrica
...
def recommend_thresholds(results: list, metric: str = "roi") -> None:
    ...
    Apresenta 3 perfis de risco: conservador, balanceado, agressivo.

    A métrica de ordenação dentro de cada perfil depende de --metric:
      roi:     ordena por ROI $ (legacy, depende de SimulatedMarket)
      outcome: ordena por outcome_score (win% × log(trades/ano), honesto)
    """
    ...
    # Métrica de ordenação dentro dos pools
    metric_col = "outcome_score" if metric == "outcome" else "roi_pct"

    # Os três perfis:
    #
    # CONSERVADOR: alta certeza antes de entrar. Menos trades, win-rate mais alto.
    #   Critério: melhor métrica entre configs com win >= 85% E threshold >= 0.60
    #
    # BALANCEADO: equilíbrio entre volume e precision.
    #   Critério: melhor métrica entre configs com >= 50 trades E |lag mediano| <= 1.0h
    #
    # AGRESSIVO: mais trades, aceita mais risco.
    #   Critério: melhor métrica global
    ...
    # Fallbacks se algum perfil ficou vazio
    ...
    if metric == "outcome":
        cols = ["Perfil", "Thr", "HMin", "Trades/ano", "Win%", "Lag med",
                "Prem%", "Score"]
    else:
        cols = ["Perfil", "Thr", "HMin", "Trades/ano", "Win%", "Lag med",
                "ROI%", "Max Draw"]
    ...
    # Bloco de código pronto a copiar
    _console.print("\n[bold]Código para munich_phased_entry.py (classe SingleEntry):[/bold]\n")
    for label, r, emoji in [
        ...
        _console.print(
            ...
            f"[dim]{stats_str}[/dim]\n"
            f"  self.threshold  = {r['threshold']:.3f}\n"
            f"  self.hour_min   = {r['hour_min']}       [dim]# não entrar antes desta hora[/dim]\n"
        )

    _console.print(
        "[dim]Recomendação geral: começar com BALANCEADO em paper por 2-3 semanas.\n"
        "Se o win-rate real se mantiver próximo do reportado, escalar para AGRESSIVO.\n"
        "Se o win-rate cair ou houver drawdown mau, descer para CONSERVADOR.[/dim]"
    )
...
# ══════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Calibração de thresholds Munich")
    ...
    parser.add_argument("--realistic", action="store_true",
                        help="Usa SimulatedMarket com ruído (recomendado — mais próximo da realidade)")
    parser.add_argument("--top", type=int, default=30,
                        help="Top-N resultados a mostrar (default 30)")
    parser.add_argument("--metric", choices=["roi", "outcome"], default="roi",
                        help="roi: optimiza ROI $ via SimulatedMarket (default, mantém comportamento). "
                             "outcome: optimiza win-rate puro sem inventar preços — métrica honesta para "
                             "ordenar configs sem depender do simulador.")
    args = parser.parse_args()

    # Em modo 'outcome' o SimulatedMarket é ignorado — não precisamos de preços
    # para calcular win/lose/premature/late. Aviso o user se passou --realistic.
    ...
    _console.print("\n[bold cyan]Munich Calibration — análise de thresholds[/bold cyan]\n")
    ...
    signals_df = generate_daily_signals(df, models)
    _console.print(f"  [green]✓[/green] {len(signals_df):,} sinais gerados")

    print_p_distribution(signals_df)
    ...
    results = run_grid_search(signals_df, thresholds, hour_mins,
                              realistic_market=args.realistic)

    _console.print(f"\n[5/5] Análise de resultados...")

    # Determinar coluna de ordenação consoante a métrica
    ...
    if args.metric == "outcome":
        ...
        notes = (
            ...
            f"  • Bracket alvo = round(running_max) — igual ao que o live bot faz\n"
