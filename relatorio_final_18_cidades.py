#!/usr/bin/env python3
"""
Relatório final completo - Backtest histórico 2010-2026 (18 cidades)
"""
import json
...
def generate_final_report():
    ...
    print("=" * 80)
    print("RELATÓRIO FINAL - BACKTEST HISTÓRICO COMPLETO 2010-2026 (18 CIDADES)")
    print("=" * 80)
    
    # Dados históricos completas
    cities_data = {
        ...
        "miami": {"days": 5966, "correct": 65.2, "premature": 39.4, "missed": 31.5, "lag": 0.83, "pnl": 6988.11, "sharpe": 22.64},
        ...
        "chicago": {"days": 5967, "correct": 33.8, "premature": 21.2, "missed": 63.7, "lag": 0.74, "pnl": 2450.42, "sharpe": 14.10},
        ...
        "phoenix": {"days": 5969, "correct": 82.3, "premature": 55.2, "missed": 12.7, "lag": 0.79, "pnl": 6273.69, "sharpe": 16.11},
        "las_vegas": {"days": 5966, "correct": 79.9, "premature": 58.4, "missed": 14.3, "lag": 0.85, "pnl": 6419.69, "sharpe": 15.10},
    ...
    print(f"Retorno total: {(total_pnl/18000)*100:.1f}% (com $1,000 por cidade)")
    ...
    print(f"\nPortfólio completo de 18 cidades:")
    print(f"   Capital: $18,000")
    print(f"   PnL esperado: ${total_pnl:,.2f}")
    print(f"   Retorno: {(total_pnl/18000)*100:.1f}%")
    
    print(f"\n🎯 CIDADES COM PERFORMANCE EXCEPCIONAL:")
    print("-" * 40)
    
    # Cidades com PnL > $6,000
    exceptional_cities = [(c, d) for c, d in cities_data.items() if d["pnl"] > 6000]
    for city, data in exceptional_cities:
        city_name = city.replace("_", " ").title()
        print(f"   🌟 {city_name}: ${data['pnl']:.2f} PnL, {data['sharpe']:.2f} Sharpe, {data['correct']:.1f}% win rate")
    
    print(f"\n🔥 NOVAS CIDADES ADICIONADAS (vs teste anterior):")
    print("-" * 45)
    
    # Comparar com o teste anterior de 14 cidades
    previous_cities = {"munich", "ankara", "singapore", "jakarta", "kuala_lumpur", "lagos", 
                      "taipei", "karachi", "moscow", "warsaw", "beijing", "madrid", "tel_aviv", "buenos_aires"}
    new_cities = set(cities_data.keys()) - previous_cities
    ...
    print(f"\n📈 MELHORES NOVAS ADIÇÕES:")
    print("-" * 35)
    
    # Ordenar novas cidades por performance
    sorted_new_cities = sorted(new_cities, key=lambda x: cities_data[x]["pnl"], reverse=True)
    for city in sorted_new_cities:
        city_name = city.replace("_", " ").title()
        data = cities_data[city]
        print(f"   🏆 {city_name}: ${data['pnl']:.2f} PnL, {data['sharpe']:.2f} Sharpe")
    
    print(f"\n🎯 RECOMENDAÇÃO FINAL:")
    print("-" * 30)
    print("🚀 Portfólio ótimo (Top 5):")
    top_5_names = [city.replace("_", " ").title() for city, _, _ in portfolio_cities[:5]]
    for i, city_name in enumerate(top_5_names, 1):
        print(f"   {i}. {city_name}")
    
    print(f"\n💡 Estratégia sugerida:")
    print("   - Começar com portfólio de 5 cidades top")
    print("   - Adicionar gradualmente mais cidades conforme capital disponível")
    print("   - Monitorar performance e ajustar conforme necessário")
    print("   - Considerar remover cidades com Sharpe < 15 após 3 meses")
