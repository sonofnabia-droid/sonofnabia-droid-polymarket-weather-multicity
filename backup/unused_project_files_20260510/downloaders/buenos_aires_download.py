""""
buenos_aires_download.py
==================
Descarrega histórico horário para Buenos Aires (SAEZ - Ezeiza) via Wunderground API
...
def fetch_day(station: str, tz: str, day: date) -> list[dict]:
    ...
    url = f"https://api.weather.com/v1/location/{station}:9:AR/observations/historical.json"
    params = {"apiKey": "e1f10a1e78da46f5b10a1e78da96f525", "units": UNITS, "startDate": date_str}
    try:
        ...
        for o in obs:
            ...
            if gmt:
                ...
                dt_utc = None
            rows.append({
                "date": date_local, "time_local": time_local, "timestamp_utc": dt_utc,
                "temp_c": o.get("temp"), "dewpt_c": o.get("dewpt"), "humidity_pct": o.get("rh"),
                "pressure_hpa": o.get("pressure"), "wind_dir_deg": o.get("wdir"),
                "wind_dir_card": o.get("wdir_cardinal"), "wind_speed_kmh": o.get("wspd"),
                "wind_gust_kmh": o.get("gust"), "precip_mm": o.get("precip_hrly"),
                "condition": o.get("wx_phrase"), "uv_index": o.get("uv_index"),
                "visibility_km": o.get("vis"), "sky_cover": o.get("sky_cover"),
                "heat_index_c": o.get("heat_index"), "windchill_c": o.get("windchill"),
            })
...
# ── Update mode ────────────────────────────────────────────────────────────────
def update_mode():
    csv_path = OUTPUT_DIR / "buenos_aires.csv"
...
def main_download(start_date: date, end_date: date):
    print(f"{Fore.CYAN}╔══════════════════════════════════════╗")
    print(f"{Fore.CYAN}║ Wunderground – SAEZ Buenos Aires (Ezeiza) ║")
    print(f"{Fore.CYAN}╚══════════════════════════════════════╝\n")
    total_days = (end_date - start_date).days + 1
    print(f"{Fore.WHITE}A descarregar {total_days} dias ({start_date} → {end_date})\n")
    with Progress(TextColumn("[bold cyan]{task.description}"), BarColumn(), TextColumn("{task.percentage:>3.0f}%"), TimeElapsedColumn(), TimeRemainingColumn()) as progress:
        task = progress.add_task(f"[SAEZ] Buenos Aires", total=total_days)
        download_station(STATION, CITY, TZ, start_date, end_date, progress, task)

# ── CLI ────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download histórico Wunderground Buenos Aires")
    parser.add_argument("--start", default="2010-01-01", help="Data de início. Aceita: 2026-01-01 ou 01-01-2026")
    parser.add_argument("--end", default=None, help="Data de fim (opcional)")
    parser.add_argument("--update", action="store_true", help="Acrescentar apenas dias novos")
