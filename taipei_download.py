...
taipei_download.py
==================
Descarrega histórico horário para Taipei (RCSS) via Wunderground API
...
def fetch_day(station: str, tz: str, day: date) -> list[dict]:
    ...
    url = f"https://api.weather.com/v1/location/{station}:9:TW/observations/historical.json"
    params = {
...
# ── Update mode ────────────────────────────────────────────────────────────────
def update_mode():
    csv_path = OUTPUT_DIR / "taipei.csv"
...
def main_download(start_date: date, end_date: date):
    print(f"{Fore.CYAN}╔══════════════════════════════════════╗")
    print(f"{Fore.CYAN}║ Wunderground – RCSS Taipei (Songshan) ║")
    print(f"{Fore.CYAN}╚══════════════════════════════════════╝\n")
    ...
    with Progress(
        ...
        task = progress.add_task(f"[RCSS] Taipei", total=total_days)
        download_station(STATION, CITY, TZ, start_date, end_date, progress, task)

# ── CLI ────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download histórico Wunderground Taipei")
