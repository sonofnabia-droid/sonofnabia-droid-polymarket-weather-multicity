...
def _bootstrap_state_today(state: CityState) -> None:
    ...
    # Se ainda não temos slots suficientes, tentar Open-Meteo
    if len(slots) < 4:
        print(f"  {C['cyan']}{city.name}: Tentando Open-Meteo bootstrap{R}")
        try:
            om_series, om_slots = bootstrap_om_today(city, state.om_sess)
            if len(om_slots) >= 4:
                series, slots = om_series, om_slots
                print(f"  {C['green']}{city.name}: Open-Meteo bootstrap OK ({len(slots)} slots){R}")
        except Exception as e:
            print(f"  {C['red']}{city.name}: Open-Meteo bootstrap falhou: {e}{R}")
...
def _tick_city(state: CityState, trading_mode_str: str, bankroll: float) -> DailyStats:
    ...
    if city.icao and state.wu_key:
        try:
            ...
            print(f"  {C['yellow']}{city.name}: V3 ICAO current falhou: {e}{R}")
    ...
    # Último fallback: Open-Meteo
    if new_obs is None:
        try:
            # Usar Open-Meteo para dados atuais
            om_data = fetch_om_hourly_today(city, state.om_sess)
            if om_data and len(om_data) > 0:
                latest = om_data[-1]
                new_obs = {
                    "temp_c": latest.get("temp_c", 20.0),
                    "humidity": latest.get("humidity", 70),
                    "pressure_hpa": latest.get("pressure_hpa", 1013),
                    "wind_speed_kmh": latest.get("wind_speed_kmh", 5),
                    "wind_dir_deg": latest.get("wind_dir_deg", 0),
                    "wind_gust_kmh": latest.get("wind_gust_kmh", 8),
                    "dewpoint_c": latest.get("dewpoint_c", 10.0),
                    "uv_index": latest.get("uv_index", 3),
                    "cloud_cover": latest.get("cloud_cover", 50),
                    "hour": latest.get("hour", datetime.now().hour),
                    "minute": 0,
                }
                print(f"  {C['cyan']}{city.name}: Open-Meteo current OK ({new_obs['temp_c']}°C){R}")
        except Exception as e:
            print(f"  {C['red']}{city.name}: Open-Meteo current falhou: {e}{R}")
