...
jakarta_download.py
==================
Descarrega histórico horário para Jakarta (WIHH - Halim Perdanakusuma) via Wunderground.

Output: jakarta_historical.csv

Colunas no CSV (idênticas ao Munich):
  datetime, temp_c, cloud_cover, humidity,
  date, hour, month, doy, year

Uso:
    python jakarta_download.py
    python jakarta_download.py --start 2014-01-01
    python jakarta_download.py --update
"""
...
def fetch_day(station: str, tz: str, day: date) -> list[dict]:
    ...
    url = f"https://api.weather.com/v1/location/{station}:9:ID/observations/historical.json"
    params = {
...
# ── Update mode ────────────────────────────────────────────────────────────────
def update_mode():
    csv_path = OUTPUT_DIR / "jakarta.csv"
...
def main_download(start_date: date, end_date: date):
    print(f"{Fore.CYAN}╔══════════════════════════════════════╗")
    print(f"{Fore.CYAN}║ Wunderground – WIHH Jakarta (Halim) ║")
    print(f"{Fore.CYAN}╚══════════════════════════════════════╝\n")
    ...
    with Progress(
        ...
        task = progress.add_task(f"[WIHH] Jakarta", total=total_days)
        download_station(STATION, CITY, TZ, start_date, end_date, progress, task)

# ── CLI ────────────────────────────────────────────────────────────────────────
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download histórico Wunderground Jakarta")
    ...
    # Parse start date (suporta os dois formatos)
    try:
        if len(args.start.split("-")[0]) == 2:   # DD-MM-YYYY
